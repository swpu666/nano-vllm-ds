...
简历只放上面 10 条结论（前 5 条量化、后 5 条投机解码），下面这套是嘴上展开的"新增了什么 / kernel 怎么写 / 为什么"。
...
**多候选（tree）验证：已评估但暂未做**（体现判断力，不是交付项，故不进简历正文）
- **评估/原型做了什么**：draft 第 0 步按 draft 分布的 **top-K 分叉**（不是独立采样 K 次 —— 同分布下独立采样的 K 条链会在第一个 token 就重合，实测 K=2 接受长度 5.95 反而低于 K=1 的 6.19）；verify 把 K 条链当成 B×K 个 varlen 序列一次算完，取接受最长的那条。greedy 下 K=2 仍与基线**逐 token 相同（4/4）**。
