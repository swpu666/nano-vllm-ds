...
# 两条执行路径 (环境变量在 import 时读取; 默认 fused):
#
#   fused (默认)  Triton dequant-GEMM, fp16 权重完全不物化 (对标 Marlin, 真正的"重写 GEMM")。
#                显存 5.2GiB (0.5B/param, 量化的真实收益), 吞吐 ~77 tok/s,
#                Part C 与 vLLM(gptq_marlin) 64/64 对齐。
#                与 cuBLAS 相比: prefill(M>=32) 逐位一致, decode(M<=4) 有 1~4 ulp 差异,
#                但这点差异**不足以**造成贪心分歧 (实测仍 64/64)。
#   torch         旧朴素实现 (torch 张量运算解包), 访存量约为权重的 30 倍, 仅作对照/兜底
#                (Triton 不可用时)。
...
try:
    ...
    ORDERED_MAX_M = 0
    _HAS_TRITON = False
_FUSED = _HAS_TRITON   # Triton 可用时 fused 是默认路径
